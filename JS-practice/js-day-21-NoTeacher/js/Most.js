...
//跟数字有关的变化框架
var yd={
	move:function(div, target, fn) {
    clearInterval(div.timer);   //先清除之前的定时器。分对象的。只关注当前对象是否有定时器。
    div.timer = setInterval(function() {  //div 对象；div.timer 在对象上创建了一个动态属性。
        ...
        for (attr in target) {
            ...
            //获取当前属性值  //兼容
            if (attr == 'opacity') {
                var iCur = Math.round(parseFloat(ys.getStyle(div, attr)) * 100);
            } else {
                var iCur = Math.round(parseFloat(ys.getStyle(div, attr)));
            }
            if(!iCur){
                iCur=0;
            }
            ...
            if (iCur == target[attr]) { //当前属性等于 目标值，运动可以结束。
                //currentCount++;
                /*clearInterval(div.timer); //清理当前对象下面的定时器
                ...
                if (attr == 'opacity') {
                    div.style.filter = 'alpha(opacity:' + parseInt(iCur + iSpeed) + ')';
                    div.style.opacity = (iCur + iSpeed) / 100;
                } else {
                    div.style[attr] = (iCur + iSpeed) + 'px';
                }
            }
        }
